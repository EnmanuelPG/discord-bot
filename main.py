...
async def handle_create_order(request):
    from cogs.tickets import create_ticket_channel, send_embed_to_pedidos
    ...
    member = guild.get_member_named(usuario)
    if not member:
        lower = usuario.lower().lstrip('@')
        for m in guild.members:
            if m.name.lower() == lower or m.display_name.lower() == lower:
                member = m
                break
    ...
    if not member:
        return web.json_response({"ok": False, "error": "USER_NOT_FOUND", "message": f"El usuario '{usuario}' no está en el servidor de Discord."}, status=400)
    ...
    if not created:
        return web.json_response({
            ...
            "message": f"El canal {ticket_channel.mention} ya existía para este ticket.",
        })

    sent = await send_embed_to_pedidos(
        guild, bot.user, ticket_id, service_name, detalle, metodo, usuario, ticket_channel
    )
